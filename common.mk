...
# $(osd_dir)/interfaces/common/dii_channel.sv \ \\ use sed add first line!!
#########################################################################################
...
osd_vsrcs := \
	...
	$(glip_dir)/backend_tcp/logic/dpi/GlipTcp.h \
	$(glip_dir)/backend_tcp/logic/dpi/GlipTcp.cpp \
	$(glip_dir)/backend_tcp/logic/dpi/glip_tcp_dpi.cpp \
	...
	$(glip_dir)/common/logic/scaler/verilog/glip_downscale.sv \
	$(glip_dir)/common/logic/scaler/verilog/glip_upscale.sv \
	$(glip_dir)/common/logic/fifo/verilog/fifo_dualclock_fwft.sv \
	$(glip_dir)/common/logic/fifo/verilog/fifo_dualclock_standard.sv \
	$(glip_dir)/common/logic/fifo/verilog/fifo_singleclock_fwft.sv \
...
########################################################################################
$(osd_common_files):
	$(foreach file,$(osd_vsrcs),echo "$(file)" >> $@;)

$(sim_common_files): $(sim_files) $(osd_common_files) $(sim_top_blackboxes) $(sim_harness_blackboxes)
	awk '{print $1;}' $^ | sort -u | grep -v '.*\.\(svh\|h\)$$' > $@
	sed -i '/interfaces\/common\/dii_channel.sv/d' $@
	sed -i "1 i $(osd_dir)/interfaces/common/dii_channel.sv" $@
...
#########################################################################################
EMPTY=$(RISCV_TEST)/empty/empty.riscv
$(output_dir):
	mkdir -p $@

$(output_dir)/%: $(RISCV_TEST)/isa/% $(output_dir)
	ln -sf $< $@
...
$(output_dir)/%.run: $(output_dir)/% $(sim)
	(set -o pipefail && $(NUMA_PREFIX) $(sim) $(PERMISSIVE_ON) $(SIM_FLAGS) +loadelf=$< $(EXTRA_SIM_FLAGS) $(SEED_FLAG) $(PERMISSIVE_OFF) $(EMPTY) </dev/null | tee $<.log) && touch $@

$(output_dir)/%.out: $(output_dir)/% $(sim)
	(set -o pipefail && $(NUMA_PREFIX) $(sim) $(PERMISSIVE_ON) $(SIM_FLAGS) +loadelf=$< $(EXTRA_SIM_FLAGS) $(SEED_FLAG) $(VERBOSE_FLAGS) $(PERMISSIVE_OFF) $(EMPTY) </dev/null 2> >(spike-dasm > $@) | tee $<.log)
